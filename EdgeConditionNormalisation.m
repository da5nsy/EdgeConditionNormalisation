...
% modify response such that it is affected in some way by the attractor
...
for i = 1:size(responses,1)
    in(i) = inpolygon(responses(i,1), responses(i,2),...
        gamut(:,1), gamut(:,2));
    if ~in(i)
        [~, response_in_x(i), response_in_y(i)] = p_poly_dist(responses(i,1), responses(i,2),...
            gamut(:,1), gamut(:,2),1);
    else
...
quiver(cues(:,1),cues(:,2),...
    response_in_x'-cues(:,1),...
    response_in_y'-cues(:,2),...
    'AutoScale','off','ShowArrowHead','off') % scaling off

%% Plot mean vectors

%% Normalise (additive/multiplicative?)
